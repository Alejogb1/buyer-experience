...
const SearchBar = () => {
    ...
    const { data: postsQuery, isFetching: isFetchingPosts } = api.category.all.useQuery<Post[]>({categoryTitle: query}, { enabled: query.length > 0});
    const { data: usersQuery, isFetching: isFetchingProducts } = api.product.all.useQuery({username: query}, { enabled: query.length > 0});
    ...
    return (
        <>
            <Select
                ...
                isLoading={ (query.length > 0 && (isFetchingPosts || isFetchingProducts || isFetchingSubreddits)) ? true : false }
                components={{
